...
Public Sub AnalyzeOPRCH()
    ...
    wsSummary.Range("A1:Q1").Value = Array( _
        "Станция", "Генератор", "Тип", "Старт", "P0, МВт", "Pтек, МВт", "Δf, Гц", "Δfр, Гц", _
...
Private Sub WriteGeneratorSheet(ByVal wsRaw As Worksheet, ByVal st As TSettings, ByVal g As TGenCfg, ByVal res As TGenResult)
    ...
    ws.Range("D1:E8").Value = Array( _
        ...
        Array("Δf, Гц", res.Df), _
        Array("Δfр, Гц", res.Dfr), _
    ...
    ws.Range("A10:F10").Value = Array("Время", "Частота, Гц", "P, МВт", "ΔPфакт, МВт", "Pтреб_накоп, МВт", "Δfр, Гц")
    ...
    AddSeries chartObj.Chart, ws, 11, outR - 1, 1, 4, "ΔPфакт, МВт", False
...
NextCfg:
    ...
    ws.Range("A10:E10").Value = Array("Время", "Частота, Гц", "Pсум, МВт", "ΔPсум, МВт", "Pтреб_сум, МВт")
    rowQ = 11
    ...
    AddSeries chartObj.Chart, ws, 11, rowQ - 1, 1, 4, "ΔPсум, МВт", False
